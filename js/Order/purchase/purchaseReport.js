...
$(document).ready(function () {
	...
	$.ajax({
		...
		data: { session_id: user_session_id, action: action, chsm: chsm },
		success: function (responseData) {
			const storeList = document.getElementById("purchaseorderStore");
			const defaultOption = document.createElement("option");
			defaultOption.text = "請選擇門市";
			defaultOption.value = "";
			storeList.appendChild(defaultOption);
...
function fetchAccountList() {
	...
	// 組裝菜單所需資料
	var action = "getPurchaseReportList";
	var chsmtoGetPurchaseList = user_session_id + action + "HBAdminReportApi";
	var chsm = CryptoJS.MD5(chsmtoGetPurchaseList).toString().toLowerCase();

	$("#partsOrderReport").DataTable();
	// 发送API请求以获取数据
	$.ajax({
		type: "POST",
		url: `${apiURL}/report`,
...
function updatePageWithData(responseData) {
	var dataTable = $("#partsOrderReport").DataTable();
	dataTable.clear().destroy();
	var data = responseData.returnData;
	// console.log(data);

	table = $("#partsOrderReport").DataTable({
		autoWidth: false,
		columns: [
			{
...
// 監聽欄位變動
$(document).ready(function () {
	var sdateValue, edateValue, storeValue, statusValue, selectedBrandId, storeValue, storeorderValue;
	...
	// 監聽訂單門市
	$("#purchaseorderStore").on("change", function () {
...
$(document).ready(function () {
	...
	function downloadSelectedData() {
		...
		var action = "getPurchaseReportListCSV";
		var chsmtoGetManualList = user_session_id + action + "HBAdminReportApi";
		var chsm = CryptoJS.MD5(chsmtoGetManualList).toString().toLowerCase();
